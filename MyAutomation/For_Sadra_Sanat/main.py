...
def massage_button_fun()->None:
    ...
    top.title('پیام رسان')
    top.geometry('465x250+300+300')
    top.configure(background = color_srt ) 

    text = tk.Text(top,font=(3) ,height = 8 , width=40 )
    text.tag_configure('tag-right', justify='right')
    scroll = tk.Scrollbar(top,background='red')
    text.configure(yscrollcommand=scroll.set)
    text.place(x=1,y=10)
    
    scroll.config(command=text.yview)
    scroll.place(x = 441 ,y = 11,height = 188)

    # ! ~~~~~~~~~~~~~~~~~~ Button ~~~~~~~~~~~~~~~~~~

    send_btn = tk.Button(top,text='ارسال' ,bg='#ffbd03').place(x = 400 , y = 200)
    
    # ? ~~~~~~~~~~~~~~~~~~ Radio ~~~~~~~~~~~~~~~~~~

    # v = tk.IntVar()
    # tk.Radiobutton(top, bg =  color_srt ,text = 'a', variable = v ,value = 1 ).place(x = 300 , y = 200)
    # tk.Radiobutton(top, bg =  color_srt ,text = 'b', variable = v ,value = 2 ).place(x = 250 , y = 200)
    ...
    for (text, value) in values.items():
                Radiobutton(top, text = text, variable = v,
                            ...
                            background = "#55c2da").place(x = 300 - index_j , y = 200)
